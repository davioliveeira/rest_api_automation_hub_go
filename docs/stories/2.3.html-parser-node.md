...
- [ ] Task 1: Create HTML Parser task structure (AC: 1)
  - [ ] Create `internal/tasks/html_parser_task.go` file
  - [ ] Define `HTMLParserTask` struct implementing `TaskExecutor` interface
  - [ ] Add Goquery package dependency to go.mod
  - [ ] Add package documentation for HTML parsing functionality

- [ ] Task 2: Implement HTML parser configuration (AC: 1)
  - [ ] Define configuration structure for HTML parsing
  - [ ] Support `html_source` field (context key containing HTML string)
  - [ ] Support `selectors` field (array of selector definitions)
  - [ ] Each selector has: `name` (output field name), `selector` (CSS selector), `attribute` (optional), `multiple` (optional boolean)

- [ ] Task 3: Implement CSS selector extraction (AC: 1)
  - [ ] Load HTML from ExecutionContext based on html_source
  - [ ] Parse HTML using Goquery
  - [ ] For each selector configuration:
    - [ ] Apply CSS selector to find element(s)
    - [ ] Extract text content or attribute value
    - [ ] Handle single vs multiple matches
  - [ ] Build result as `[]map[string]any` per AC1

- [ ] Task 4: Implement Execute method (AC: 1)
  - [ ] Validate configuration (required: html_source, selectors)
  - [ ] Load HTML content from ExecutionContext
  - [ ] Parse HTML document with Goquery
  - [ ] Execute all selectors and collect results
  - [ ] Return TaskResult with extracted data array

- [ ] Task 5: Support multiple extraction patterns (AC: 1)
  - [ ] Single element extraction (first match)
  - [ ] Multiple element extraction (all matches)
  - [ ] Attribute extraction (href, src, class, etc.)
  - [ ] Text content extraction (default)

- [ ] Task 6: Implement error handling (AC: 1)
  - [ ] Handle missing HTML source in context
  - [ ] Handle invalid HTML parsing errors
  - [ ] Handle invalid CSS selectors
  - [ ] Handle empty selector results (warn but don't fail)
  - [ ] Return descriptive error messages in TaskResult.Error
  - [ ] Log errors using `slog`

- [ ] Task 7: Store extracted data in ExecutionContext (AC: 1)
  - [ ] Return extracted data array in TaskResult.Output
  - [ ] Format as `[]map[string]any` per AC1
  - [ ] Engine will store TaskResult.Output in context
  - [ ] Data available for Transform or other tasks

- [ ] Task 8: Register HTML Parser task with Registry
  - [ ] Create initialization function `RegisterHTMLParserTask(registry *engine.Registry)`
  - [ ] Register with task type name "html_parser"
...
- [ ] Task 9: Write comprehensive unit tests (Testing requirement)
  - [ ] Create `internal/tasks/html_parser_task_test.go`
  - [ ] Test single element extraction
  - [ ] Test multiple element extraction
  - [ ] Test attribute extraction (href, src)
  - [ ] Test nested selectors
  - [ ] Test empty results handling
  - [ ] Test invalid HTML handling
  - [ ] Test missing HTML source
  - [ ] Ensure >80% test coverage

- [ ] Task 10: Integration test with HTTP and Transform tasks
  - [ ] Create workflow: HTTP fetch HTML -> Parse -> Transform -> verify
  - [ ] Test extracting data from real HTML structure
  - [ ] Verify parsed data available for transformation
